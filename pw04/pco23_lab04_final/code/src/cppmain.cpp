 ...
 *
 * File name :   cppmain.cpp
 *
 * Author(s) :   Demont Kilian et Graf Calvin
 *
 * Description : Programme principal de simulation du système ferroviaire.
 *               Ce programme crée deux locomotives, initialise les aiguillages,
 *               positionne les locomotives, et lance les threads de comportement
 *               des locomotives pour simuler leur déplacement synchronisé sur
 *               une maquette de chemin de fer.
 *
 */
...
void emergency_stop()
{
    locoA.arreter();
    locoB.arreter();
    afficher_message("ARRET D'URGENCE !");
}
...
int cmain()
{
    ...
    // Création du tableau 2D qui concerne les aiguillages de la section partagée
    unsigned int aiguillageLoco[2][3] = {{10, DEVIE, 0}, {13, DEVIE, 0}};

    // Création du thread pour la loco 0
    std::unique_ptr<Launchable> locoBehaveA = std::make_unique<LocomotiveBehavior>(locoA, sharedSection,  22, 21, 11, false, aiguillageLoco);

    // Création du thread pour la loco 1
    std::unique_ptr<Launchable> locoBehaveB = std::make_unique<LocomotiveBehavior>(locoB, sharedSection, 25, 24, 15, true, aiguillageLoco);
